      ...
      {hasHidden ? (
        <Collapsible open={open} onOpenChange={setOpen}>
          ...
          <CollapsibleTrigger
            aria-expanded={open}
            onClick={(event) => {
              // This control can sit inside a linked job card. Keep expanding
              // skills independent from the card's navigation.
            ...
            className={cn(
              "group/more mt-2 inline-flex items-center gap-1.5 rounded-full border border-dashed border-border/60 bg-accent-soft/50 py-0.5 text-[10px] font-semibold text-foreground/75 transition-all duration-200",
              open ? "px-2.5" : "pl-1 pr-2",
          ...
          >
            ...
            <span>{open ? "Hide" : `more skill${remaining === 1 ? "" : "s"}`}</span>
            <ChevronDown
              className={cn(
