...
/**
 * Scripted match events — a rotating cast that cycles through the feed so
 * the section feels alive without fake random score jitter. Each tick the
 * next event enters at the top and the oldest one leaves.
 */
const MATCH_EVENTS: Match[] = [
  { id: "e1", candidate: "A. Khan", role: "Senior Frontend · Helio", score: 94, city: "Singapore" },
  { id: "e2", candidate: "M. Okafor", role: "ML Engineer · Lumen", score: 91, city: "London" },
  { id: "e3", candidate: "S. Park", role: "Staff PM · Vertex", score: 89, city: "Seoul" },
  { id: "e4", candidate: "R. Diaz", role: "Data Engineer · Atlas", score: 87, city: "Madrid" },
  { id: "e5", candidate: "T. Yamamoto", role: "Mobile Lead · Polaris", score: 86, city: "Tokyo" },
  { id: "e6", candidate: "K. Andersson", role: "Backend Lead · Lumen", score: 84, city: "Stockholm" },
  { id: "e7", candidate: "L. Nguyen", role: "Product Designer · Helio", score: 92, city: "Ho Chi Minh City" },
  { id: "e8", candidate: "J. Mwangi", role: "iOS Engineer · Vertex", score: 88, city: "Nairobi" },
  { id: "e9", candidate: "E. Rossi", role: "DevOps Lead · Atlas", score: 85, city: "Milan" },
  { id: "e10", candidate: "P. Sharma", role: "Full-stack Dev · Polaris", score: 90, city: "Bangalore" },
];
...
export function CoverPlatform() {
  ...
  const [visible, setVisible] = useState<Match[]>(
    MATCH_EVENTS.slice(0, VISIBLE_COUNT)
  );
  const cursor = useRef(VISIBLE_COUNT);
  const feedRef = useRef<HTMLDivElement | null>(null);
  const [offscreen, setOffscreen] = useState(false);

  // Pause the feed when it scrolls out of view.
  useEffect(() => {
  ...
  // Advance the scripted feed. Paused when offscreen, when the tab is
  // hidden, and entirely for reduced-motion users (static list).
  useEffect(() => {
    if (offscreen || reducedMotion) return;
    const id = window.setInterval(() => {
      if (document.hidden) return;
      setVisible((prev) => {
        const next = MATCH_EVENTS[cursor.current % MATCH_EVENTS.length];
        cursor.current += 1;
      ...
      <ScrollReveal className="mx-auto max-w-2xl text-center">
            ...
            >
              ...
              <ul className="divide-y">
                {visible.map((m) => (
                  <li
                    key={m.id}
                    className="animate-reveal flex items-center gap-4 px-6 py-4 transition-colors hover:bg-muted/40"
                  >
        ...
        <ScrollReveal delay={260} className="mt-5">
          <div className="overflow-hidden rounded-2xl border bg-card text-card-foreground shadow-sm">
            ...
            {/* Static summary for assistive tech — the marquee itself is
                decorative, duplicated, and constantly moving. */}
            <p className="sr-only">
              Recent platform activity includes new roles posted, candidates
              moving to interview, credentials verified, and offers accepted.
            </p>
            <div
              aria-hidden
            ...
            >
              <div className="flex animate-ticker gap-8 whitespace-nowrap px-6 py-4 group-hover:[animation-play-state:paused]">
