...
export function UniversityReports() {
  ...
  return (
    <div className="space-y-6">
      ...
      {noMatches && <Card className="border-dashed"><CardContent className="flex flex-wrap items-center justify-between gap-4 p-6"><div><p className="font-medium">No graduate outcomes match this report scope.</p><p className="mt-1 text-sm text-muted-foreground">Your selected filters are still active. Reset them to return to the full graduate cohort.</p></div><Button type="button" variant="outline" onClick={resetFilters}><RotateCcw aria-hidden />Reset report filters</Button></CardContent></Card>}
